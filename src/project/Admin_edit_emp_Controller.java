...
public class Admin_edit_emp_Controller {
    ...
    public boolean checkInputs() throws IOException{
        ...
        if (email.getText().isEmpty() || accountno.getText().isEmpty()) {
            openPopup("Missing Input", "Please fill all the fields.");
            return false;
        }
   ...
   return true;
    ...
    public void handleSaveButton(ActionEvent actionEvent) throws IOException, LineUnavailableException, UnsupportedAudioFileException {
        ...
        emp.editEmployeeAccountField(emp.getEmp_id(),"email",emp.getEmail(), false);
        emp.editEmployeeAccountField(emp.getAccount_number(),"account_number",emp.getAccount_number(), false);
        emp.editEmployeeAccountField(emp.getEmp_id(), "name", emp.getName(), false);
